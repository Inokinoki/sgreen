...
emit_case "UC40" "Quiet wipe with no sessions (-q -wipe)" "" "$SGREEN_BIN -q -wipe" "$SCREEN_BIN -q -wipe"
emit_case "UC41" "Power detach missing named session with command (-D name cmd)" "" "$SGREEN_BIN -D nosuch /bin/sh -c 'echo hi'" "$SCREEN_BIN -D nosuch /bin/sh -c 'echo hi'"
emit_case "UC42" "Send command to missing named session (-X ... -S name)" "" "$SGREEN_BIN -X quit -S nosuch" "$SCREEN_BIN -X quit -S nosuch"
emit_case "UC43" "Detached no-fork start (-D -m -S name cmd)" "" "$SGREEN_BIN -D -m -S dmcase /bin/sh -c 'exit 0'" "$SCREEN_BIN -D -m -S dmcase /bin/sh -c 'exit 0'"
emit_case "UC44" "Title flag with list (-t title -ls)" "" "$SGREEN_BIN -t mytitle -ls" "$SCREEN_BIN -t mytitle -ls"
emit_case "UC45" "Login flag on with list (-l -ls)" "" "$SGREEN_BIN -l -ls" "$SCREEN_BIN -l -ls"
emit_case "UC46" "Login flag off with list (-ln -ls)" "" "$SGREEN_BIN -ln -ls" "$SCREEN_BIN -ln -ls"
emit_case "UC47" "PTY quiet reattach with no session (-q -r)" "" "script -q /dev/null $SGREEN_BIN -q -r" "script -q /dev/null $SCREEN_BIN -q -r"
emit_case "UC48" "Owner/session reattach requires suid root (-r owner/session)" "" "$SGREEN_BIN -r otheruser/tty.host" "$SCREEN_BIN -r otheruser/tty.host"
emit_case "UC49" "Owner/session multiattach requires suid root (-x owner/session)" "" "$SGREEN_BIN -x otheruser/tty.host" "$SCREEN_BIN -x otheruser/tty.host"
